...
def run(args, device, data):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
            ...
            train_time = (time.time() - tic_step)

            iter_tput.append(len(seeds) / train_time)
            if step % args.log_every == 0:
                ...
                print(
                    "Epoch {:05d} | Step {:05d} | Loss {:.4f} | Train Acc {:.4f} | Time {:.4f} | Speed (samples/sec) {:.4f} | GPU {:.1f} MB".format(
                        epoch,
                        step,
                        loss.item(),
                        acc.item(),
